...
const ThumbnailGallery = ({ images, setMainImage }: { images: Venues["media"], setMainImage: (url: string) => void }) => {
  return (
    <div className="h-full flex flex-col">
      <div className={`grid gap-1 h-full ${images.length === 2 ? 'grid-rows-2' : 'grid-rows-2 md:grid-rows-auto md:grid-cols-2'}`}>
...
function MediaGallery({ images }: MediaGalleryProps) {
    ...
    const [mainImage, setMainImage] = useState(images.length > 0 ? images[0].url : 'https://via.placeholder.com/600');
    ...
    return (
      <div className="relative w-full h-[500px] md:h-[600px] overflow-hidden">
        ...
        <div className="hidden md:grid grid-cols-3 gap-4 h-full">
          ...
          {thumbnailCount > 0 && <ThumbnailGallery images={thumbnailImages} setMainImage={setMainImage} />}
        </div>
      </div>
    );
  }
