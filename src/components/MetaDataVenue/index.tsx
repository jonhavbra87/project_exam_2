...
function MetaDataVenue({ meta }: { meta: { wifi: boolean; parking: boolean; breakfast: boolean; pets: boolean } }) {
    ...
    <div className="mb-4 text-lg">
      ...
      <ul className="flex gap-4">
        ...
        {metaData
          ...
          .map((item) => (
            <li key={item.key} className="text-2xl text-primary">{item.icon}</li>
          ))}
      </ul>
    </div>
  );
