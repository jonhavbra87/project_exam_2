...
const ProfileEditForm = () => {
  ...
  const onSubmit = async (data: { avatar: string; banner: string; venueManager: boolean }) => {
    const updatedData = {
      avatar: data.avatar ? { url: data.avatar, alt: profile?.avatar?.alt || "User Avatar" } : profile?.avatar,
      banner: data.banner ? { url: data.banner, alt: profile?.banner?.alt || "User Banner" } : profile?.banner,
  ...
  return (
    ...
    >
      ...
      {/* 🔹 Email (Read-only) */}
      <label className="block mt-3 text-sm font-semibold text-gray-700">E-mail</label>
      <div className="relative flex items-center">
      ...
      {/* 🔹 Avatar (Editable) */}
      <label className="block mt-3 text-sm font-semibold text-gray-700">Avatar URL</label>
      <div className="relative flex items-center">
      ...
      {/* 🔹 Banner (Editable) */}
      <label className="block mt-3 text-sm font-semibold text-gray-700">Banner URL</label>
      <div className="relative flex items-center">
        <FaImage className="absolute left-3 text-gray-500" />
        <input
          {...register("banner")}
